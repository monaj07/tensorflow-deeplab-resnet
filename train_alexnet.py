...
def load_pretrained(data_path, session, ignore_missing=False):
...
def main():
    ...
    # Image summary.
    images_summary = tf.py_func(inv_preprocess, [image_batch, args.batch_size, IMG_MEAN], tf.uint8)
    labels_summary = tf.py_func(decode_labels, [label_batch, args.batch_size, args.num_classes], tf.uint8)
    preds_summary = tf.py_func(decode_labels, [pred, args.batch_size, args.num_classes], tf.uint8)

    total_summary = tf.summary.image('images',
                                     tf.concat(axis=2, values=[images_summary, labels_summary, preds_summary]),
                                     max_outputs=args.batch_size)  # Concatenate row-wise.
    ...
    load_pretrained('bvlc_reference_caffenet.npy', sess)
    ...
    for step in range(args.num_steps):
        ...
        feed_dict = {step_ph: step}#, do_rate: 0.5}
